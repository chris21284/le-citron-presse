<template>
    <header :class="isShrinked ? 'shrink-header':''">
        <router-link to="/">
            <img src="@/assets/icons/logo_onlylemon.png" alt="LogoCitronPressé" class="logo">
            <img src="@/assets/icons/title_white.png" alt="Le Citron Pressé" class="titre" :class="[isShrinked ? 'shrink-title': '']">
        </router-link>
...
<script>
    import CartBadge from "@/views/Component/CartBadge.vue"
    import HalftoneLayer from "@/views/SingleElement/HalftoneLayer.vue";
    ...
    export default {
        ...
        computed: {
            halftoneSize() {
                let res = this.isShrinked ? (this.halftoneBaseSize / 1.6).toString() : this.halftoneBaseSize.toString();
                return res + "rem";
            }
        },
        methods: {
