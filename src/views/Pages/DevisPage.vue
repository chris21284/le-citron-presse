<template>
    <div class="devis-page">
        ...
        <div class="side-info">
            ...
            <InfoBubble 
                class="infoBubble"
                msg="Pour le moment les commandes en lignes seront probablement à récupérer sur le marché d'Aubagne pour certains produits dont la livraison est pour le moment compliqué."
            />
...
<script>
    import DevisItem from "@/views/SingleElement/DevisItem.vue";
    import InfoBubble from "@/views/SingleElement/InfoBubble.vue";

    export default {
        name: 'DevisPage',
        components: { DevisItem, InfoBubble },
        data() {
            return {
                cart: {},
                numberOfElements: 0,
        ...
        computed: {
            getTotalPrice() {
                ...
                for (let id in this.cart) {
                    ...
                    if (!item.price) return '- - (Prix manquant)'; //si pour le moment, l'article n'a aucun prix
...
<style scoped>
    ...
    @media only screen and (max-width: 580px) {
        .infoBubble { top: -30%; left: 0% }
    }

    @media only screen and (max-width: 550px) {
        .side-info { margin-top: 3rem; }
    }
</style>